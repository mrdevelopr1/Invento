...
class PurchaseController extends Controller
{
    ...
    public function PrintPurchase($id) {
        ...
        $purchase = Purchase::findOrFail($id);
        // Fetch all purchases with the same purchase number
        $relatedPurchases = Purchase::where('purchase_no', $purchase->purchase_no)->get();
        return view('backend.pdf.purchase_pdf', compact('purchase', 'relatedPurchases'));
    }
    public function PrintPurchaseList(){
        $allData = purchase::orderBy('date','desc')->orderBy('id','desc')->where('status','1')->get();
        return view('backend.purchase.invoice_print_list',compact('allData'));
    }
